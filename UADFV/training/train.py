...
parser.add_argument('--detector_path', type=str,
                    default='/data/Btask/DeepfakeBench/training/config/detector/sbi.yaml',
...
def prepare_training_data(config):
    ...
    elif config.get('use_balanced_sampler', False):
        # 使用平衡采样器
        from training.dataset.abstract_dataset import BalancedSampler  # 绝对导入

        # 获取最小样本数和是否允许重复采样
        min_samples = config['balanced_sampler'].get('min_samples_per_class', 5)
        replacement = config['balanced_sampler'].get('replacement', True)
